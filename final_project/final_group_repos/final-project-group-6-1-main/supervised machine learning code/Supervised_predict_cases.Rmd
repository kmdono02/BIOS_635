...
result_ridge <- list()
      for(f in 1:length(cv_folds_10)){
        ...
        lasso_fit = cv.glmnet(x = data.matrix(food_data[,c(1:20)]), y = data.matrix(food_data[,21]), alpha=1)
        ridge_fit = cv.glmnet(x = data.matrix(food_data[,c(1:20)]), y = data.matrix(food_data[,21]), alpha=0)
...
food_data$Deaths_vs_median <- as_factor(food_data$Deaths_vs_median)
      ...
      for(f in 1:length(cv_folds_10)){
        ...
        # lasso & ridge
        lasso_fit = cv.glmnet(x = data.matrix(food_data[,c(1:20)]), y = data.matrix(food_data[,23]), alpha=1, family="binomial")
        ridge_fit = cv.glmnet(x = data.matrix(food_data[,c(1:20)]), y = data.matrix(food_data[,23]), alpha=0, family="binomial")
