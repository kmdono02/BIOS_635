...
player_data_00 <- 
  read_csv(file = "data/nba_players_99_00.csv") %>%
  filter(MP>=1000)
player_data_10 <- 
  read_csv(file = "data/nba_players_09_10.csv") %>%
  filter(MP>=1000)
player_data_20 <- read_csv(file = "data/nba_players_19_20.csv") %>%
  filter(MP>=1000)
```
...
```{r 2a_answer}
player_datasets <- list("1999-2000"=player_data_00, 
                        "2009-2010"=player_data_10, 
                        "2019-2020"=player_data_20)

for(i in 1:length(player_datasets)){
  player_datasets[[i]] <- 
    player_datasets[[i]] %>%
    mutate(season = names(player_datasets)[i])
}
```
...
```{r 2b_answer}
player_datasets_all <- do.call("rbind", player_datasets)

# Compare dimensions of combined and individual datasets
dim(player_datasets_all)

print("rows")
dim(player_datasets$`1999-2000`)[1]+dim(player_datasets$`2009-2010`)[1]+
  dim(player_datasets$`2019-2020`)[1]

print("columns")
dim(player_datasets$`1999-2000`)[2]
dim(player_datasets$`2009-2010`)[2]
dim(player_datasets$`2019-2020`)[2]
```
...
```{r 3a_answer, include=TRUE}
tbl_summary(data=team_data,
            by=playoffs,
            include=c("W", "L", "Age", "SOS", "ORtg", "DRtg",
